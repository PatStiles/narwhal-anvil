        ...
        if let Some(tx_chain_id) = tx.chain_id() {
            ...
            if chain_id != tx_chain_id.into() {
                if let Some(legacy) = tx.as_legacy() {
                    ...
                    {
                        warn!(target: "backend", "incompatible EIP155-based V; chain_id: {:?}, tx_chain_id: {:?}", chain_id, tx_chain_id);
                        return Err(InvalidTransactionError::IncompatibleEIP155);
                    }
                } else {
                    warn!(target: "backend",  "invalid chain id; chain_id: {:?}, tx_chain_id: {:?}", chain_id, tx_chain_id);
