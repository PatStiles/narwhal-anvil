...
use anvil_core::{
    ...
    types::{Forking, Index, NodeEnvironment, NodeForkConfig, NodeInfo, Work},
};
...
use ethers::{
    ...
    types::{
        ...
        GethDebugTracingOptions, Log, Trace, Transaction, TxHash, H256, U256, U64,
    },
    utils::rlp,
};
...
use worker::batch_maker::Transaction as Tx;
...
pub struct EthApi {
    ...
    net_listening: bool,
    /// Channel to send Tx to worker batch_maker in Narwhal Mempool
    tx_worker_channel: Sender<Tx>,
    /// Store path to narwhal mempool
    store: Store,
    /// listeners for new ready transactions
    tx_listeners: Arc<Mutex<Vec<Snd<Tx>>>>,
}
...
impl EthApi {
    ...
    pub fn new(
        ...
        Self {
            ...
            tx_worker_channel,
            store,
            tx_listeners: Arc::new(tx_listeners),
        }
    }
    ...
    pub async fn execute(&self, request: EthRequest) -> ResponseResult {
        ...
        match request {
            ...
            */
            _ => ResponseResult::Error(RpcError::invalid_request()),
        }
    }
    ...
    async fn block_request(&self, block_number: Option<BlockId>) -> Result<BlockRequest> {
        let block_request = match block_number {
            Some(BlockId::Number(BlockNumber::Pending)) => BlockRequest::Number(U64::zero()),
            _ => {
    ...
    */
    ...
    pub async fn send_transaction(&self, request: EthTransactionRequest) -> Result<TxHash> {
        ...
        let pool_transaction =
            self.add_pending_transaction(pending_transaction, requires, provides).unwrap();
        self.tx_worker_channel
            .send(serde_json::to_vec(&pool_transaction).unwrap())
            .await
            .expect("Failed to Deliver Serialized PoolTransaction to Worker");
        trace!(target: "node", "Added transaction: [{:?}] sender={:?}", pool_transaction.hash(), from);
        Ok(*pool_transaction.hash())
    }
...
impl EthApi {
    ...
    /// transaction
    pub fn add_ready_listener(&self) -> Receiver<Tx> {
        const TX_LISTENER_BUFFER_SIZE: usize = 2048;
        let (tx, rx) = channel(TX_LISTENER_BUFFER_SIZE);
        self.tx_listeners.lock().push(tx);
        rx
    }
    ...
    */
    ...
    ) -> Result<PoolTransaction> {
        let from = *pending_transaction.sender();
        let priority = self.transaction_priority(&pending_transaction.transaction);
        Ok(PoolTransaction { requires, provides, pending_transaction, priority })
    }
